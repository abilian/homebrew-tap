...
- **Source builds inherit the upstream's compiler floor.** pyobjc-core (11.1 through at least 12.2.2) compiles a test extension with `-fdisable-block-signature-string`, which no Xcode that runs on macOS 14 accepts (Apple clang 16 and older), while its wheels install anywhere. CI on current macOS never sees it. `patches/pyobjc-core.diff` drops the flag; the generator inlines any `patches/<resource>.diff` into that resource.
